<template>
  <div class="editor-container">
    ...
    <!-- 知识图谱模态框 -->
    <div
      v-if="showKnowledgeGraphModal"
      class="modal-overlay"
      @click="closeKnowledgeGraph"
      style="
        position: fixed;
        top: 0;
        left: 0;
  ...
  try {
    ...
    for (const oldRef of oldReferences) {
      ...
      if (!newRef) {
        ...
        await appService.initialize(currentVaultId)
...
const handleEditorDrop = async (event: DragEvent) => {
  ...
  console.log(
    '[MarkdownEditor] Fragment drag check, fragmentId:',
    fragmentId,
    'dataTransfer:',
    event.dataTransfer,
  )
  ...
  try {
    ...
    if (currentFilePath.value) {
        ...
        (async () => {
          try {
            ...
            if (electronAPI && electronAPI.file && electronAPI.file.getFileCache) {
              ...
              // 检查是否已存在该引用（同时检查 fragmentId 和 position）
              const existingIndex = cache.references.findIndex(
                (r: any) => r.fragmentId === fragmentId && r.position === newPosition,
              )
              const cacheEntry = {
                fragmentId,
      ...
      asyncOperations.push(
        (async () => {
          try {
            console.log('[MarkdownEditor] 开始注册外部文件引用, fragmentId:', fragmentId)
            ...
            // 优先从 URL route query 获取 vaultId，如果没有则使用全局变量
            const routeVaultId = (route?.query?.vaultId as string) || (window as any).__vaultId__
            const currentVaultId = routeVaultId || app.getCurrentVaultId() || 'default'
            console.log('[MarkdownEditor] 使用 vaultId:', currentVaultId)

            await app.getApplicationService().initialize(currentVaultId)

            // 直接创建服务实例（不需要从容器获取）
            const { FragmentReferenceRegistrationService } = await import(
              '../../domain/services/fragment-reference-registration.service'
            )
            const { StorageAdapter } = await import('../../infrastructure/storage.adapter')

            // 创建服务实例，传入正确的 vaultId
            const documentRepository = StorageAdapter.createDocumentRepository()
            const fragmentRepository =
              StorageAdapter.createKnowledgeFragmentRepository(currentVaultId)

            const registrationService = new FragmentReferenceRegistrationService(
              documentRepository,
              fragmentRepository,
            )
            console.log('[MarkdownEditor] 创建 registrationService:', registrationService)
            ...
            console.log(
              '[MarkdownEditor] 调用 registerExternalFileReference, fileDocumentId:',
              fileDocumentId,
            )
            ...
            console.log('[MarkdownEditor] 外部文件引用注册完成')
          } catch (error) {
      ...
      asyncOperations.push(
        (async () => {
          try {
            ...
            console.log('[MarkdownEditor] 引用注册完成')
          } catch (error) {
