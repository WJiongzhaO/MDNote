 ...
 * 知识片段组合式函数
 */
export function useKnowledgeFragments() {
  const fragments = ref<KnowledgeFragmentResponse[]>([]);
  const isLoading = ref(false);
  const error = ref<string | null>(null);
  const fragmentUseCases = ref<ReturnType<typeof Application.getInstance>['getKnowledgeFragmentUseCases'] | null>(null);

  // 异步获取fragmentUseCases
  const application = Application.getInstance();
  const initFragmentUseCases = async () => {
    try {
      // 确保应用服务已初始化
   ...
   */
  const insertFragmentToDocument = async (fragmentId: string, documentId: string): Promise<string> => {
    if (!fragmentUseCases.value) {
      await initFragmentUseCases();
    }
    if (!fragmentUseCases.value) {
