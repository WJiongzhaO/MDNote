...
export class ApplicationModule {
  static configure(container: ServiceContainer): void {
    ...
    // 注意：KnowledgeFragmentRepository 不再通过DI绑定，而是在 KnowledgeFragmentUseCases 中直接创建
    container
    ...
    // 注意：KnowledgeHealthService 不再通过 DI 绑定，直接在 ApplicationService 中创建
    ...
    // 配置编辑器工具栏服务
    container.bind<FormatEditorService>(TYPES.FormatEditorService).toSingleton(FormatEditorService)

    container
      .bind<InsertContentService>(TYPES.InsertContentService)
      .toSingleton(InsertContentService)

    container
      .bind<EditorToolbarUseCase>(TYPES.EditorToolbarUseCase)
    ...
    container.bind<CommandExecutor>(TYPES.CommandExecutor).toSingleton(CommandExecutor)

    // 1.5 ShortcutCommandsFactory - 依赖 EditorToolbarUseCase
    container
      .bind<ShortcutCommandsFactory>(TYPES.ShortcutCommandsFactory)
      .toSingleton(ShortcutCommandsFactory)

    // 2. ShortcutRegistry - 依赖仓储
    container.bind<ShortcutRegistry>(TYPES.ShortcutRegistry).toSingleton(ShortcutRegistry)

    // 3. ConflictDetector - 依赖 ShortcutRegistry
    container.bind<ConflictDetector>(TYPES.ConflictDetector).toSingleton(ConflictDetector)

    // 基础设施服务
    container
      .bind<KeyboardEventProcessor>(TYPES.KeyboardEventProcessor)
      .toSingleton(KeyboardEventProcessor)

    container.bind<PlatformAdapter>(TYPES.PlatformAdapter).toSingleton(PlatformAdapter)

    // 4. ShortcutManager（门面） - 最后绑定，依赖上述所有服务
    container.bind<ShortcutManager>(TYPES.ShortcutManager).toSingleton(ShortcutManager)
    ...
    Promise.resolve().then(() => {
      try {
        ...
        const keyboardProcessor = container.get<KeyboardEventProcessor>(
          TYPES.KeyboardEventProcessor,
        )
