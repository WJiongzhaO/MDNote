 ...
 * 引用标志解析器
 * 支持三种形态的容器化语法：
 * - 形态A（Linked）: {{ref:id:linked}} 或 {{ref:vaultId/id:linked}} 或 [知识片段：标题]
...
export class FragmentReferenceParser {
  ...
  // 支持跨知识库格式：{{ref:vaultId/fragmentId:mode}}
  private readonly REF_PATTERN =
    /\{\{ref:(([a-zA-Z0-9-]+\/)?[a-zA-Z0-9-]+)(?::(linked|detached|connected|disconnected))?\}\}/g
  ...
  parseReferences(content: string): FragmentReferenceMatch[] {
    ...
    while ((match = this.REF_PATTERN.exec(content)) !== null) {
      ...
      const modeStr = match[3] || 'linked' // 默认为linked模式
      ...
      if (fullId.includes('/')) {
        ...
        vaultId = parts[0] ?? undefined
        fragmentId = parts.slice(1).join('/')
      } else {
        fragmentId = fullId
      }
