...
import {
  ...
  type FragmentReferenceMatch,
} from './fragment-reference-parser.service'
import { crossVaultFragmentService } from '../../application/services/cross-vault-fragment.service'
  ...
  async resolveReferences(content: string, documentId: string, fileCache?: any): Promise<string> {
    ...
    for (const ref of sortedReferences) {
      ...
      try {
        ...
        const vaultId = ref.vaultId

        // 获取对应知识库的仓储
        const repository = await this.getFragmentRepository(vaultId)

        // 优先从缓存读取（如果提供了缓存）
        if (fileCache && fileCache.references) {
          const cachedRef = fileCache.references.find(
            (r: any) => r.fragmentId === ref.fragmentId && r.isConnected,
          )
          if (cachedRef && cachedRef.content) {
            // 检查缓存是否有效：比较片段更新时间
            const fragment = await repository.findById({ value: ref.fragmentId })
            if (fragment) {
              ...
              if (fragmentUpdatedAt <= cacheUpdatedAt) {
                ...
                console.log(
                  `[FragmentReferenceResolver] Using cached content for fragment ${ref.fragmentId}`,
                )
              } else {
                // 片段已更新，需要重新处理
                console.log(
                  `[FragmentReferenceResolver] Fragment ${ref.fragmentId} updated, refreshing cache`,
                )
          ...
          if (imagePaths.length > 0) {
            try {
              ...
              pathMap.forEach((newPath, oldPath) => {
                ...
                fragmentMarkdown = fragmentMarkdown!.replace(
