...
export class FileSystemDocumentRepository implements DocumentRepository {
  ...
  async save(document: Document): Promise<void> {
    ...
    const documentData: DocumentData = {
      ...
      fragmentReferences: document.getFragmentReferences(),
    }
  ...
  async findByFolderId(folderId: FolderId): Promise<Document[]> {
    const documentsData = (await this.getAllDocumentsFromFile()).filter(
      (data) => data.folderId === folderId.value,
    )
