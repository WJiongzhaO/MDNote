...
export class KnowledgeFragmentUseCases {
  ...
  async createFragment(
    request: CreateKnowledgeFragmentRequest,
  ): Promise<KnowledgeFragmentResponse> {
    let nodes = request.nodes.map((nodeData) => createNodeFromJSON(nodeData))

    // 创建知识片段（临时，用于获取ID）
    const fragment = KnowledgeFragment.create({ value: request.title }, nodes, request.tags || [])
   ...
   * 片段内容会在预览和导出时自动解析
   */
  async insertFragmentToDocument(fragmentId: string, documentId: string): Promise<string> {
        ...
        if (sourcePath.startsWith('./assets/')) {
          ...
          try {
            ...
            if (sourceFilePath || isExternalFileId) {
              ...
              // 无法确定源路径
              console.warn(
                '无法确定图片源路径，sourceDocumentId和sourceFilePath都未提供:',
                fileName,
              )
              // 保持原节点，不复制图片
              updatedNodes.push(node)
   ...
   */
  private extractPreview(nodes: ASTNode[]): {
    previewImage?: string
    previewType?: 'image' | 'mermaid'
    previewMermaidCode?: string
  } {
    const traverse = (
      nodeList: ASTNode[],
    ): {
      ...
      for (const node of nodeList) {
        ...
        if (node.type === NodeType.CODE_BLOCK) {
          ...
          const isMermaid =
            (codeBlock.language && codeBlock.language.toLowerCase() === 'mermaid') ||
            (codeBlock.isMermaid &&
              typeof codeBlock.isMermaid === 'function' &&
              codeBlock.isMermaid())
  ...
  async listFragments(params: ListFragmentsParams): Promise<KnowledgeFragmentResponse[]> {
    ...
    if (params.keyword?.trim()) {
      ...
      list = list.filter(
        (f) =>
          f.getTitle().value.toLowerCase().includes(q) ||
          f.getTags().value.some((t) => t.toLowerCase().includes(q)) ||
          f.toMarkdown().toLowerCase().includes(q),
      )
    }
    if (params.tags?.length) {
      list = list.filter((f) => params.tags!.every((t) => f.getTags().value.includes(t)))
    }
    if (params.statuses?.length) {
      list = list.filter((f) => params.statuses!.includes(f.getStatus()))
    }
    if (params.categoryIds?.length) {
      list = list.filter((f) =>
        f.getCategoryPathIds().some((id) => params.categoryIds!.includes(id)),
      )
    }
    if (params.recentUpdated) {
      list = [...list].sort((a, b) => b.getUpdatedAt().getTime() - a.getUpdatedAt().getTime())
    }
    if (params.recentUsed) {
      list = [...list].sort((a, b) => {
        const aMax = Math.max(...a.getReferencedDocuments().map((r) => r.referencedAt.getTime()), 0)
        const bMax = Math.max(...b.getReferencedDocuments().map((r) => r.referencedAt.getTime()), 0)
        return bMax - aMax
      })
    }
    ...
    list = [...list].sort((a, b) => {
      let cmp = 0
      if (sortBy === 'updatedAt') cmp = a.getUpdatedAt().getTime() - b.getUpdatedAt().getTime()
      else if (sortBy === 'referencedAt') {
        const aMax = Math.max(...a.getReferencedDocuments().map((r) => r.referencedAt.getTime()), 0)
        const bMax = Math.max(...b.getReferencedDocuments().map((r) => r.referencedAt.getTime()), 0)
        cmp = aMax - bMax
      } else if (sortBy === 'trustScore') cmp = a.getTrustScore() - b.getTrustScore()
      else if (sortBy === 'title') cmp = a.getTitle().value.localeCompare(b.getTitle().value)
      return order === 'asc' ? cmp : -cmp
    })

    return list.map((f) => this.mapToResponse(f))
  }

  /** 工作2：批量更新状态/分类/标签 */
  async bulkUpdateFragments(params: BulkUpdateFragmentsParams): Promise<void> {
    for (const id of params.ids) {
      const fragment = await this.repository.findById({ value: id })
      if (!fragment) continue
      if (params.status !== undefined) fragment.updateStatus(params.status)
      if (params.categoryPathIds !== undefined) fragment.updateCategories(params.categoryPathIds)
      if (params.tagsToAdd?.length) params.tagsToAdd.forEach((tag) => fragment.addTag(tag))
      if (params.tagsToRemove?.length) params.tagsToRemove.forEach((tag) => fragment.removeTag(tag))
      await this.repository.save(fragment)
    }
  }

  /** 工作2：片段详情（资产视角） */
  async getFragmentDetail(id: string): Promise<KnowledgeFragmentDetailResponse | null> {
    const fragment = await this.repository.findById({ value: id })
    if (!fragment) return null
    const base = this.mapToResponse(fragment)
    return {
      ...base,
      source: fragment.getSource(),
      trustScore: fragment.getTrustScore(),
      ...
      status: fragment.getStatus(),
      derivedFromId: fragment.getDerivedFromId(),
      versionHistory: fragment.getVersionHistory(),
    }
  }

  /** 工作2：仅更新元数据 */
  async updateFragmentMetadata(
    id: string,
    patch: UpdateFragmentMetadataRequest,
  ): Promise<KnowledgeFragmentResponse> {
    const fragment = await this.repository.findById({ value: id })
    if (!fragment) throw new Error(`Knowledge fragment with id ${id} not found`)
    if (
    ...
    if (patch.status !== undefined) fragment.updateStatus(patch.status)
    if (patch.categoryPathIds !== undefined) fragment.updateCategories(patch.categoryPathIds)
    await this.repository.save(fragment)
    return this.mapToResponse(fragment)
  }
